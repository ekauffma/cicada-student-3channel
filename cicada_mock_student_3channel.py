...
def generate_random_dataset(n_samples: int) -> np.ndarray:
    ...
    channel0 = np.random.randint(0, 256, size=(n_samples, 252, 1), dtype=np.uint8)

    # Channels 1 and 2: binary 0/1
    channel1 = np.random.randint(0, 2, size=(n_samples, 252, 1), dtype=np.uint8)
    channel2 = np.random.randint(0, 2, size=(n_samples, 252, 1), dtype=np.uint8)

    bin = np.concatenate([channel1, channel2], axis=-1).astype("float32")

    return (channel0.astype("float32"), bin)

# Example usage:
X_main, X_bin = generate_random_dataset(num_samples) 
print(X_main.shape)  # (num_samples, 252, 1)
print(X_main.dtype)  
print(X_bin.shape)  # (num_samples, 252, 2)
print(X_bin.dtype) 

class Cicada3Channel:
    def __init__(self, input_shape: tuple):
        self.input_shape = input_shape

    def get_model(self):
        input_main = Input(shape=self.input_shape, name="input_main")
        input_bin = Input(shape=self.input_shape + (2,), name="input_bin")

        input_main_expanded = Reshape((252,1), name="reshape1")(input_main)
        x = Concatenate(axis=-1, name="concat_inputs")([input_main_expanded, input_bin])
        x = Reshape((18, 14, 3), name="reshape2")(x)
        ...
        #     padding="valid",
        #     use_bias=False,
        #     depthwise_quantizer=quantized_bits(12, 3, 1, alpha=1.0),
        #     name="depthwise_conv",
        # )(x)

        # # Pointwise (1x1) conv to mix channels → 4 filters
        # x = QConv2D(
        #     4,
        #     (1, 1),
        #     use_bias=False,
        ...
        x = QConv2D(
            4,                 # output channels same as pointwise conv
            (2, 2),            # combine kernel sizes of depthwise+pointwise
            strides=2,         # same as depthwise stride
            padding="valid",
            use_bias=False,
            kernel_quantizer=quantized_bits(12, 3, 1, alpha=1.0),
            name="conv_replacement",
        )(x)
        ...
        return Model([input_main, input_bin], outputs, name="cicada-3channel")


cicada_3channel = Cicada3Channel((252,)).get_model()
cicada_3channel.compile(optimizer=Adam(learning_rate=0.001), loss="mae")
...
def get_hls_config(keras_model):
    ...
    # # ---- INPUT PRECISION (Mixed Precision) ----
    hls_config["LayerName"]["input_main"]["Precision"]["result"] = "fixed<10,6>"
    hls_config["LayerName"]["input_bin"]["Precision"]["result"] = "fixed<1,1>"
...
hls_config = get_hls_config(cicada_3channel)
...
hls_model = convert_to_hls4ml_model(cicada_3channel, hls_config)
